...
def certify(model, device, dataset, num_classes, matfile=None, mode='hard',
            start_img=0, skip=1, sigma=0.25, N0=100, N=100000, alpha=0.001, batch=1000, verbose=False,
            grid=(0.25, 0.50, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0, 2.25), beta=1.0, logger=None, do_pbar=False):
    assert skip > 0
    # If we want to log the Certification Summary
    if logger is not None:
        print_cmd = logger.info
    else:
        print_cmd = print
    print_cmd('===certify(N={}, sigma={}, mode={})==='.format(N, sigma, mode))
    print(grid)

    model.eval()
    smoothed_net = Smooth(model, num_classes, sigma, device, mode, beta)

    num_img = int(len(dataset)/skip)
    ...
    if do_pbar: pbar = tqdm.tqdm(total=num_img, leave=False)
    ...
    for i in range(num_img):
        ...
        if mode == 'both':
            p_hard, r_hard, p_soft, r_soft = smoothed_net.certify(img, N0, N, alpha, batch)
        ...
        if do_pbar: pbar.update(1)

    if do_pbar: pbar.close()

    print_cmd('===Certify Summary===')
    print_cmd('Total Image Number: {}'.format(num_img))
    if mode == 'both':
        print_cmd('===Hard certify===')
        print_cmd('Radius: 0.0  Number: {}  Acc: {}'.format(
            cnt_grid_hard[0], cnt_grid_hard[0] / num_img * 100))
        for j in range(num_grid):
            print_cmd('Radius: {}  Number: {}  Acc: {}'.format(
                grid[j], cnt_grid_hard[j + 1], cnt_grid_hard[j + 1] / num_img * 100))
        print_cmd('ACR: {}'.format(s_hard / num_img))
        print_cmd('===Soft certify===')
        print_cmd('Radius: 0.0  Number: {}  Acc: {}'.format(
            cnt_grid_soft[0], cnt_grid_soft[0] / num_img * 100))
        for j in range(num_grid):
            print_cmd('Radius: {}  Number: {}  Acc: {}'.format(
                grid[j], cnt_grid_soft[j + 1], cnt_grid_soft[j + 1] / num_img * 100))
        print_cmd('ACR: {}'.format(s_soft / num_img))
